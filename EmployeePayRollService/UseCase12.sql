...
('Marine','2019-8-14','f','99998888','bella'),
('Tim','2018-8-14','f','99998444','grim'),
('tina','2019-8-14','m','999998444','rim');

select * from  Employee1

insert into Payroll1 values
('2','2500','2000','200','250','2450'),
('3','2400','2000','200','250','2450'),
('4','300','2000','200','250','2450');

select * from Payroll1

SELECT  p.basicpay, e.name FROM Payroll1 p inner join Employee1 e on p.Emp_id=e.emp_id where e.name='tina';


select sum(p.basicpay) as "sum",e.gender from payroll1 p inner join Employee1 e on p.emp_id=e.Emp_id  group by gender ;
select max(p.basicpay) as "maxSalary",e.gender from payroll1 p inner join Employee1 e on p.emp_id=e.Emp_id  group by gender ;
select min(p.basicpay) as "minSalary",e.gender from payroll1 p inner join Employee1 e on p.Emp_id=e.Emp_id  group by gender ;
select avg(p.basicpay) as "averageSalry",e.gender from payroll1 p inner join Employee1 e on p.emp_id=e.Emp_id  group by gender ;
select count(p.basicpay) as "count",e.gender from payroll1 p inner join Employee1 e on p.Emp_id=e.Emp_id  group by gender ;



SELECT e.name,e.startdate,e.gender,e.phone,e.adress,c.company_name,s.basicpay FROM Employee1 as e  inner join Company1 AS c  ON e.Emp_id=c.Emp_id
inner join Payroll1 AS s on e.emp_id=s.emp_id inner join  Department1 as d on d.dept_id=e.emp_id inner join Employee_Dept1 as ed on ed.dept_id=d.dept_id
